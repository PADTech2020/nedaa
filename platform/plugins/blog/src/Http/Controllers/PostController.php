...
class PostController extends BaseController
{
    ...
    public function store(
        ...
        $post->content=ExtractTweetUrl($post->content);
        $post->content=ExtractFacebookpost($post->content);
        $post->content=ExtractFacebookvideo($post->content);
        $post->content =  preg_replace("#<br\s*/?>#i", "", $post->content);
        $post->short_link=generateShortcode($post->id,8);
    ...
    public function update(
        ...
        $post->published_at=date('Y-m-d', strtotime($published_at));
